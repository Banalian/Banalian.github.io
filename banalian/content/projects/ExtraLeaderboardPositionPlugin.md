...
À l'aide de ce plugin, vous pouvez afficher plusieurs positions supplémentaires (comme par exemple la 10éme, 100éme, 1000éme, etc...) dans le classement. Il est également possible d'afficher les positions hypothétiques des médailles de la course.
...
- Toutes les X minutes (paramétrable), le plugin récupère les données du classement de la course en cours, selon les paramètres de l'utilisateur. (quelles positions, médailles, etc...)
    - Optionnellement, il est possible d'utiliser une API externe pour réaliser ces tâches, afin d'avoir accès à plus de données (voir la page de [Extra Leaderboard API](/projects/extraleaderboardapi)).
